...
import { getAccessToken, refreshToken, logout, apiFetch } from '@/auth';
...
async function submitConfirmation() {
    ...
    try {
        const transactionData = {
            address: confirmAddress.value,
            amount: confirmAmount.value,
            currency: confirmCurrency.value,
            type: 'deposit',
        };
        ...
        if (!res.ok) {
            ...
            throw new Error(errorData.detail || 'Failed to submit confirmation.');
        }
        ...
        messageTimeout = setTimeout(() => {
            ...
            // Show error message
        messageVisible.value = true;
        messageText.value = e.message || 'An error occurred.';
        messageType.value = 'error';
        clearTimeout(messageTimeout);
        messageTimeout = setTimeout(() => {
            messageVisible.value = false;
