...
const currencies = ['BTC', 'ETH', 'USDT TRC20']
...
const fee = computed(() => {
    return amount.value > 0 ? (amount.value * 0.01).toFixed(2) : '0.00'
})
...
async function submitWithdrawal() {
    // Validation
    if (!selectedCurrency.value || !walletAddress.value || !amount.value || amount.value <= 0) {
        message.value = 'Please select a currency, enter a valid address, and a positive amount.';
    ...
    try {
        const transactionData = {
            ...
            type: 'withdraw',
            // We need a field in the backend to store this
        ...
        if (!res.ok) {
            ...
            throw new Error(errorData.detail || 'Failed to submit withdrawal request.');
        }
        ...
        // You might want to show a confirmation status or redirect the user
        setTimeout(() => {
            confirmStatus.value = true; // Or handle as needed
        }, 2000);

    } catch (e) {
        message.value = e.message || 'An error occurred.';
        messageType.value = 'error';
        clearTimeout(messageTimeout);
        messageTimeout = setTimeout(() => {
            message.value = '';
